        ...
        {queueItems.length === 0 ? (
          ...
          >
            {paginatedQueueItems.map((refundRequest) => (
              <button
                aria-pressed={refundRequest.id === activeSelectedRefundId}
                className={
                  refundRequest.id === activeSelectedRefundId
                    ? 'queue-row queue-row--selected'
                    : 'queue-row'
                }
                key={refundRequest.id}
                onClick={() => setSelectedRefundId(refundRequest.id)}
                type="button"
              >
          ...
          >
            ...
            <div className="detail-actions">
              ...
              {actionOverrides[selectedRefund.id] ? (
                ...
                >
                  {statusLabel(actionOverrides[selectedRefund.id] ?? 'pending')}
                </Badge>
              ) : null}
            </div>
          </Card>
